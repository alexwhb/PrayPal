...
export async function action({ request }: Route.ActionArgs) {
	...
	const newMessage = await prisma.message.create({
		...
		include: {
			sender: {
				select: { id: true, username: true, name: true, image: { select: { id: true } } }
			}
		}
	})
...
export default function ConversationPage({loaderData, actionData}: Route.ComponentProps) {
	...
	useEffect(() => {
		...
		const handleConnectionAck = (data: { userId: string, socketId: string }) => {
					...
					>
						<Avatar className="h-8 w-8">
							{participant.image?.id ? (
								<AvatarImage
									src={getUserImgSrc(participant.image.id)}
