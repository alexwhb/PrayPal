...
export default function JoinMixerPage({
	...
	const profile =
		isUpdate && profileId ? (mockProfiles as any)[profileId] : null
			...
			{children.map((child, index) => (
				<div key={index}>
					<input
						...
						value={child.name}
					/>
					<input type="hidden" name={`child-age-${index}`} value={child.age} />
				</div>
			))}
				...
				<Dialog open={searchDialogOpen} onOpenChange={setSearchDialogOpen}>
					...
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>
								Search for church members to add to this profile.
							</DialogDescription>
						</DialogHeader>
							...
							<CommandGroup>
								{activityTypes.map((activity) => (
									<CommandItem
										...
										onSelect={() => {
											const newSelected = selectedActivities.includes(
												activity.id,
											)
...
function ChildrenSection({
	...
	setChildren: React.Dispatch<
		React.SetStateAction<Array<{ name: string; age: string }>>
	>
}) {
	return (
		<div>
	...
	return (
		<div className="flex flex-row items-center justify-between rounded-lg border p-4">
			<div className="space-y-0.5">
				<Label htmlFor="canHost" className="text-base">
					Willing to Host
				</Label>
				<p className="text-muted-foreground text-sm">
					Are you willing to host activities at your home?
				</p>
			</div>
			<Switch
	...
	return (
		<div className="space-y-4">
			<div>
				...
				<p className="text-muted-foreground mb-4 text-sm">
					Select the dates you're available for activities over the next two
					months.
				</p>
				<Calendar
					mode="multiple"
