...
export default function GroupDetailView({
	...
	const visibleMembers = canViewAllMembers ? group.members : leaders
							...
							<div className="flex items-center gap-2">
								...
								{group.isOnline && (
									<Button variant="ghost" size="sm" className="h-6 px-2">
										<Icon name="external-link" size="xm" className="mr-1" />
										Join
									</Button>
								)}
							</div>
						...
						{canViewAllMembers ? (
							...
							>
								...
								<DialogContent className="max-h-[80vh] overflow-y-auto">
									...
									{leaders.length > 0 && (
										<>
											<h4 className="mt-4 mb-2 font-medium">Leaders</h4>
											<div className="space-y-2">
												{leaders.map((member) => (
													<div
														key={member.id}
									...
									{canViewAllMembers && members.length > 0 && (
										<>
											<h4 className="mt-4 mb-2 font-medium">Members</h4>
											<div className="space-y-2">
												{members.map((member) => (
													<div
														key={member.id}
				...
				{group.isMember && (
					<Button variant="outline" size="sm" asChild>
						<Link to={`/messages/new?groupId=${group.id}`}>
							<Icon name="message-square" className="mr-2" size="sm" />
							Message Group
						</Link>
					</Button>
				)}
